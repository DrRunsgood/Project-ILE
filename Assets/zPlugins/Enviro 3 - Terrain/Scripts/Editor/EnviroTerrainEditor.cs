    ...
    public class EnviroTerrainEditor : Editor
    {
       ...
       private SerializedProperty rainFlowIntensity, rainFlowDistortionScale, rainFlowDistortionStrength, rainFlowTiling, rainFlowStrength, rainFlowSmoothnessBoost;
        ...
        public override void OnInspectorGUI()
        {
            ...
            GUILayout.Space(5);
            GUILayout.Label("Version: 1.2.1", headerStyleMid);
            ...
            if(myTarget.showLayers)
            {
                ...
                for(int i = 0; i < myTarget.layers.Count; i++)
                {
                    ...
                    EditorGUILayout.Space(5f); 
                    myTarget.layers[i].snow = EditorGUILayout.Slider("Snow",myTarget.layers[i].snow, 0f,1f); 
                    myTarget.layers[i].wetness = EditorGUILayout.Slider("Wetness",myTarget.layers[i].wetness, 0f,1f); 
                    myTarget.layers[i].puddles = EditorGUILayout.Slider("Puddles",myTarget.layers[i].puddles, 0f,1f); 
                     EditorGUILayout.Space(5f); 
                    }
                    GUILayout.EndVertical();
                }
